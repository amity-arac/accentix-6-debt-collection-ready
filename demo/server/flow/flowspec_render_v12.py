...
# Logic must stay byte-identical to src/aax6/core/flowspec_render.py. Only imports are adapted.
...
from demo.server.flow.flowspec_v12 import load_flow_spec, validate_flow_spec
...
def render_catalog(catalog: list[dict]) -> str:
    """Flow-mode catalog listing — MUST show the template TEXT per entry.

    บทเรียน v4 gate: compact listing (intent label อย่างเดียว) ทำให้ template
    ต่างเนื้อหาแต่ intent เดียวกัน (แจ้งยอด+ถาม vs ถามเปล่า) แยกไม่ออกจาก prompt
    — ยุค id ตายตัว model จำ id ได้ แต่ flow-mode remap id ตลอด จึงต้องอ่านจาก
    ข้อความเท่านั้น (unlearnable ถ้าไม่ render ข้อความ)"""
    lines = ["## Available Pre-Scripts (เลือก text_id จากรายการนี้เท่านั้น)"]
    by_state: dict[str, list[dict]] = {}
    for e in catalog:
        by_state.setdefault(e.get("state", "other"), []).append(e)
    for state in sorted(by_state):
        lines.append(f"\n### {state}")
        for e in sorted(by_state[state], key=lambda x: x["text_id"]):
            lines.append(f"- **{e['text_id']}** ({e['_fine_state']}): {e['template']}")
    return "\n".join(lines)


def default_output_path(spec: dict, version: str = RENDER_VERSION) -> Path:
    return (__import__('pathlib').Path(__file__).resolve().parents[3] / "data/system_instructions/pre-script"
