...
| `AAX6_STT_STREAMING` | `0` | Use **true Chirp `streaming_recognize`** for the STT final instead of post-speech batch — audio uploads *during* speech, so the final can land sooner at end-of-speech. A/B it against the default (`0`): watch the control-bar **STT (perceived)** number over ~10 mic turns each. Thai interim partials are historically sparse (they'll render if Chirp emits them), but the streaming *final* is what this tests. Silero still owns endpointing + barge-in; when on, the batch interim/final workers don't run. If STT is equal/higher, Chirp buffers Thai internally before finalizing — revert to `0`. |
