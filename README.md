...
| `AAX6_TTS_CACHE` | `1` | In-process TTS cache (keyed by exact reply text) + prewarm. Leave on for the demo. **Set `0` for latency benchmarking**: a repetitive clip set makes the agent emit near-identical replies whose cached audio (an instant one-blob hit) understates true TTS/TTFA — `0` forces a real cold synth every turn. In-turn prefetch+play fan-out is unaffected. The `/api/tts` `Server-Timing: cache` header (and the E2E harness's hit-rate line) report the state either way. |
